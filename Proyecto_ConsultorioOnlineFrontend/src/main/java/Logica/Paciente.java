...
public class Paciente {
    ...
    public boolean guardarPaciente(){
        ...
        String sentencia = "INSERT INTO paciente (identificacion_paciente,"
                + "tipo_identificacion, nombres, apellidos, genero, edad, direccion,"
    ...
    public boolean borrarPaciente(int identificacion){
        ConexionBD conexion = new ConexionBD();
        String sentencia = "DELETE FROM `paciente` WHERE `identificacion_paciente` ='" + identificacion + "'";
        if(conexion.setAutoCommit(false)){
    ...
    public boolean actualizarPaciente(){
        ConexionBD conexion = new ConexionBD();
        String sentencia = "UPDATE `paciente` SET nombres='" + this.nombres + "',apellidos='" + this.apellidos + "',genero='" + this.genero
                + "',tipo_identificacion='" + this.tipo_identificacion + "',telefono='" + this.telefono + "',direccion='" + this.direccion + "',edad='" + this.edad
                +  "' WHERE identificacion_paciente=" + this.identificacion_paciente + ";";
        if(conexion.setAutoCommit(false)){
            if(conexion.acualizarBD(sentencia)){
                conexion.commitBD();
                conexion.cerrarConnection();
    ...
    public Paciente getContacto() throws SQLException {
        ConexionBD conexion = new ConexionBD();
        String sql = "select * from paciente where identificacion_paciente='" + this.identificacion_paciente + "'";
        ResultSet rs = conexion.consultarBD(sql);
        if (rs.next()) {
            this.identificacion_paciente = rs.getInt("identificacion_paciente");
            this.nombres = rs.getString("nombres");
            this.apellidos = rs.getString("apellidos");
            this.genero = rs.getString("genero");
            this.tipo_identificacion = rs.getString("tipo_identificacion");
